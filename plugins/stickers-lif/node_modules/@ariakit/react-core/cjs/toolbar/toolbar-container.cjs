"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _ZKEGT5BQcjs = require('../__chunks/ZKEGT5BQ.cjs');


var _HADU3YYRcjs = require('../__chunks/HADU3YYR.cjs');
require('../__chunks/D3SPQ2OY.cjs');
require('../__chunks/FXRXBA2Y.cjs');
require('../__chunks/FMI5QF7N.cjs');
require('../__chunks/KLZID3QO.cjs');


var _WSW5XX7Fcjs = require('../__chunks/WSW5XX7F.cjs');
require('../__chunks/H7R5D2FR.cjs');
require('../__chunks/NHZBX2RS.cjs');
require('../__chunks/GQAB4M3B.cjs');
require('../__chunks/LHHW5ZQP.cjs');




var _Z5PUIG6Acjs = require('../__chunks/Z5PUIG6A.cjs');
require('../__chunks/UHRHLHM2.cjs');
require('../__chunks/5WLTMVVQ.cjs');
require('../__chunks/7WUTS26Y.cjs');



var _72I2GWXFcjs = require('../__chunks/72I2GWXF.cjs');

// src/toolbar/toolbar-container.ts
var useToolbarContainer = _Z5PUIG6Acjs.createHook.call(void 0, 
  (_a) => {
    var _b = _a, { store } = _b, props = _72I2GWXFcjs.__objRest.call(void 0, _b, ["store"]);
    const context = _WSW5XX7Fcjs.useToolbarContext.call(void 0, );
    store = store || context;
    props = _ZKEGT5BQcjs.useCompositeContainer.call(void 0, _72I2GWXFcjs.__spreadValues.call(void 0, { store }, props));
    props = _HADU3YYRcjs.useToolbarItem.call(void 0, _72I2GWXFcjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var ToolbarContainer = _Z5PUIG6Acjs.createMemoComponent.call(void 0, 
  (props) => {
    const htmlProps = useToolbarContainer(props);
    return _Z5PUIG6Acjs.createElement.call(void 0, "div", htmlProps);
  }
);
if (process.env.NODE_ENV !== "production") {
  ToolbarContainer.displayName = "ToolbarContainer";
}



exports.ToolbarContainer = ToolbarContainer; exports.useToolbarContainer = useToolbarContainer;
